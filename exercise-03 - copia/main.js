...
// Sistemas de partículas independientes para los cuadrantes
...
async function init() {
  try {
    ...
    // Configurar rangos y máximos físicos
    sliderSonido.max = 90; // dB
    sliderSonido.min = 30;
    sliderSonido.value = factores.sonido.impacto_porcentaje ? Math.round(30 + (factores.sonido.impacto_porcentaje / 100) * 60) : 65;

    sliderLuz.max = 1000; // Lux
    sliderLuz.min = 100;
    sliderLuz.value = 450;

    sliderTactil.max = 10; // mm/s²
    sliderTactil.min = 0;
    sliderTactil.value = 2.5;

    // Actualizar displays iniciales
    valSonido.textContent = sliderSonido.value;
    valLuz.textContent = sliderLuz.value;
    valTactil.textContent = sliderTactil.value;
...
// Función para crear textura circular suave (partícula esférica)
...
// Crear grupo individual por factor sensorial
...
function crearParticulas() {
  const volSonido = crearGrupoParticulas(0x2d8cff); // Azul (Sonido)
  sysSonido = volSonido.mesh;
  baseSonido = volSonido.basePos;

  const volLuz = crearGrupoParticulas(0xffdf3f); // Amarillo (Luz)
  sysLuz = volLuz.mesh;
  baseLuz = volLuz.basePos;

  const volTactil = crearGrupoParticulas(0xff3b4f); // Rojo (Táctil / Vibración)
  sysTactil = volTactil.mesh;
  baseTactil = volTactil.basePos;
}

// Cinética especializada (Decibeles, Lux, Vibración en mm/s²)
function moverGrupo(sys, base, slider, time, tipoFactor) {
  ...
  // Si es el canal Táctil, se comporta como vibración armónica de alta frecuencia
...
function configurarSliders() {
  sliderSonido.addEventListener('input', () => {
    valSonido.textContent = sliderSonido.value;
    updateParticles();
  });
  sliderLuz.addEventListener('input', () => {
    valLuz.textContent = sliderLuz.value;
    updateParticles();
  });
  sliderTactil.addEventListener('input', () => {
    valTactil.textContent = sliderTactil.value;
    updateParticles();
  });
}
...
function configurarSimulacion() {
  ...
  btnReset.addEventListener('click', () => {
    ...
    valSonido.textContent = '40';
    valLuz.textContent = '300';
    valTactil.textContent = '0.0';
...
function actualizarSimulacion() {
  ...
  // Mapeo dinámico de datos curriculares a unidades físicas reales
  const targetSonido = 30 + (niveles.sonido / 100) * 60; // 30 a 90 dB
  const targetLuz = 100 + (niveles.luz / 100) * 900;     // 100 a 1000 Lux
  const targetTactil = (niveles.tactil / 100) * 10;      // 0 a 10 mm/s²
  ...
  valSonido.textContent = Math.round(sliderSonido.value);
  valLuz.textContent = Math.round(sliderLuz.value);
  valTactil.textContent = Number(sliderTactil.value).toFixed(1);
}

// Bucle principal con renderizado en 4 vistas simultáneas (ScissorTest)
function animate() {
  ...
  // 1. Cuadrante Arriba Izquierda (Solo Sonido - dB)
  ...
  // 2. Cuadrante Arriba Derecha (Solo Luz - Lux)
  ...
  // 3. Cuadrante Abajo Izquierda (Solo Táctil - Vibración mm/s²)
  ...
  // 4. Cuadrante Abajo Derecha (Entorno Combinado Total)
